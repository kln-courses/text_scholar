...
setwd(wd)
getwd()
source("code.R/util_fun.R")# input and parse file
...
# remove stop words
length(tokens.fil.v)
stopword.v = unlist(lapply(read.csv("resources/stoplist.csv"),as.character),use.names=FALSE)
head(stopword.v)
...
save(filename.v,tokens.v,tokens.fil.v,file = 'data/somedata/kjv.RData')

## synonym substitution based on WordNet database
load('data/somedata/kjv.RData')
library("wordnet")
